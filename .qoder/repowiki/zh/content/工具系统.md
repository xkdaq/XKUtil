...
## 更新摘要
**变更内容**
- 新增配置生成器工具（ConfigGenerator），支持从结构化文本解析生成 key=value 配置
- 新增 HashCode 工具，提供 Java 风格字符串 hashCode 计算功能
- 工具注册系统已更新，包含 text 工具模块的完整集成
- 版本升级至 0.3.0，增强了工具系统的功能完整性
...
**更新** 版本 0.3.0 新增了配置生成器和 HashCode 工具，进一步完善了工具系统的功能覆盖。
...
- [src/tools/text/index.tsx:1-17](file://src/tools/text/index.tsx#L1-L17)
...
- 加密工具集：提供 AES 加密/解密、Hash 计算、密钥/IV 生成、密钥库持久化与暴力破解辅助。
- 文本处理工具集：提供配置生成器和 HashCode 计算功能。
- 状态存储：应用设置与密钥库持久化，基于 Zustand 与本地持久化中间件。

**更新** 新增了文本处理工具模块，包含配置生成器和 HashCode 工具。
...
Registry-->>Browser : 返回包含 text 工具的完整列表
...
- 关键点：
  ...
  - 分类名映射函数支持中文化显示，包含 text 类别。
  - 工具路径与组件懒加载结合，保证首屏性能。

```mermaid
flowchart TD
Start(["开始"]) --> Collect["收集 allTools<br/>合并 JSON、加密与文本工具"]
...
- 关键实现：
  ...
  - Hash 计算：模拟 Java int 32位有符号溢出的字符串哈希算法，确保与 Java 环境兼容。

```mermaid
sequenceDiagram
participant UI as "ConfigGenerator 组件"
participant Utils as "utils/config-generator.ts"
UI->>Utils : generateConfig(input)
Utils->>Utils : getRealIpViaApi(host)
Utils-->>UI : {success, data|error}
UI->>Utils : formatOutput(data)
Utils-->>UI : "key=value" 字符串
```
...
Registry-->>Routes : 工具列表包含 text 工具
Routes-->>Routes : 为每个工具生成路由
Layout->>Registry : getToolCategories()
Registry-->>Layout : 分类列表包含 text 分类
...
## 结论
XKUtil 工具系统通过注册表驱动的分类与动态路由机制，实现了清晰的模块划分与良好的扩展性。JSON、加密与文本处理三大模块分别提供了实用的数据处理、安全能力和配置生成能力，配合懒加载与状态持久化，兼顾了性能与用户体验。版本 0.3.0 新增的配置生成器和 HashCode 工具进一步完善了工具系统的功能覆盖，为开发者提供了更全面的工具集。未来可在现有基础上完善关键字搜索、工具权限与审计日志等能力，进一步增强企业级可用性。
