...
## 简介
本文件面向XKUtil项目的“加密工具系列”，聚焦于AES加解密子系统，涵盖以下主题：
...
## 项目结构
AES工具位于工具模块的“加密工具”分类下，采用按功能拆分的组件化组织方式：
- 主界面组件负责用户交互与流程控制
- 配置面板、密钥面板、密钥库抽屉分别承担参数设置、密钥输入与密钥库管理
- 工具函数封装底层加密/解密逻辑与随机数生成
- 状态管理通过轻量状态库实现本地持久化
...
subgraph "AES工具"
AesMain["AesCrypto 主界面<br/>src/tools/crypto/AesCrypto/index.tsx"]
...
sequenceDiagram
participant U as "用户"
participant M as "AesCrypto 主界面"
...
### 用户界面与交互
- 主界面包含配置面板、密钥面板、操作按钮、输入输出区域与密钥库抽屉
...
```mermaid
graph LR
AesMain["AesCrypto 主界面"] --> Utils["工具函数<br/>crypto.ts"]
...
## 结论
XKUtil的AES工具通过清晰的组件划分与完善的参数体系，提供了易用且安全的加解密能力。配合密钥库管理与暴力破解辅助功能，满足了日常开发与运维场景中的多种需求。建议在生产环境中遵循安全最佳实践，谨慎处理密钥与日志，定期评估与优化密钥库规模与访问策略。

## 附录
- 实际使用示例（步骤说明）
  - 加密：在配置面板选择模式/填充/输出格式/密钥格式/密钥长度；在密钥面板输入密钥与IV；在输入框粘贴明文；点击“加密”；复制输出
  - 解密：在配置面板保持与加密一致的参数；在密钥面板输入密钥与IV；在输入框粘贴密文；点击“解密”
