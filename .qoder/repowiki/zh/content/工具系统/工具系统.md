...
- [src/tools/color/index.tsx](file://src/tools/color/index.tsx)
- [src/tools/color/ColorPicker/index.tsx](file://src/tools/color/ColorPicker/index.tsx)
...
## 更新摘要
**变更内容**
- 新增颜色工具模块，包含 ColorPicker 工具，支持颜色选择、透明度调整、实时预览和多种输出格式
- 新增 color 分类，扩展工具注册表的分类映射
- 版本升级至 0.6.0，新增颜色工具功能，显著增强了工具系统的视觉设计能力
...
## 简介
本文件为 XKUtil 工具系统的技术文档，围绕工具注册表机制、工具分类与动态注册流程、工具接口规范、JSON 工具、加密工具、格式转换工具、颜色工具与文本处理六大模块的架构与实现进行深入解析，并提供工具开发规范、扩展指南、API 文档与集成示例，帮助开发者快速理解与扩展该工具平台。

**更新** 版本 0.6.0 新增了颜色工具模块，特别包含 ColorPicker 工具，支持颜色选择、透明度调整、实时预览和多种输出格式，进一步完善了工具系统的视觉设计能力。

## 项目结构
XKUtil 采用前端单页应用架构，工具系统位于 src/tools 下，按功能域划分为 JSON 工具、加密工具、格式转换、颜色工具与文本处理五类；通用工具函数位于 src/utils；路由与侧边菜单由布局组件负责渲染；状态管理通过 Zustand 的轻量 store 实现。
...
subgraph "颜色工具"
ColorIndex["tools/color/index.tsx"]
ColorPicker["tools/color/ColorPicker/index.tsx"]
end
subgraph "文本处理工具"
...
- [src/tools/registry.ts:1-50](file://src/tools/registry.ts#L1-L50)
...
- [src/tools/color/index.tsx:1-17](file://src/tools/color/index.tsx#L1-L17)
...
- [src/tools/color/ColorPicker/index.tsx:1-138](file://src/tools/color/ColorPicker/index.tsx#L1-L138)
...
- 颜色工具集：提供颜色选择器工具，支持颜色选择、透明度调整、实时预览和多种输出格式。
- 文本处理工具集：提供配置生成器、翻译工具和 HashCode 计算功能。
- 翻译服务系统：支持多平台翻译服务，包含配置管理、语言映射和错误处理。
- 状态存储：应用设置与密钥库持久化，基于 Zustand 与本地持久化中间件。

**更新** 新增了颜色工具模块，包含 ColorPicker 工具，支持颜色选择、透明度调整、实时预览和多种输出格式，扩展了工具系统的视觉设计能力。
...
Registry-->>Layout : 返回分类与工具列表包含颜色工具
Layout-->>Browser : 渲染侧边菜单
Browser->>Routes : 访问工具路由
Routes->>Registry : 获取全部工具
Registry-->>Browser : 返回包含 color 工具的完整列表
...
### 工具注册表与分类系统
- 职责：聚合 JSON、加密、格式转换、颜色与文本处理工具，按 category 构建分类树，提供分类与全量工具查询。
- 关键点：
  - 使用 Map 去重与聚合，确保分类唯一性与工具列表完整性。
  - 分类名映射函数支持中文化显示，包含 json、encoding、text、network、crypto、convert、color 类别。
  - 工具路径与组件懒加载结合，保证首屏性能。

```mermaid
flowchart TD
Start(["开始"]) --> Collect["收集 allTools<br/>合并 JSON、加密、格式转换、颜色与文本工具"]
...
**图表来源**
- [src/tools/registry.ts:8-32](file://src/tools/registry.ts#L8-L32)
...
### 颜色工具模块（ColorPicker）
- 工具清单：
  - 取色器：支持颜色选择、透明度调整、实时预览和多种输出格式。
- 关键实现：
  - 颜色选择：使用 Ant Design ColorPicker 组件，支持十六进制颜色值输入与选择。
  - 透明度控制：滑块控件支持 0%-100% 透明度调节，步进 10%，实时显示 Alpha 值。
  - 实时预览：棋盘格透明度背景显示，支持 RGBA 颜色预览。
  - 多格式输出：支持 #AARRGGBB（带透明度）和 #RRGGBB（纯色）两种格式。
  - 复制功能：一键复制 HEX 值和 ARGB 值，支持剪贴板 API。
  - 状态管理：使用 React hooks 管理颜色状态与透明度状态。

```mermaid
sequenceDiagram
participant UI as "ColorPicker 组件"
participant Clipboard as "Clipboard API"
UI->>UI : 选择颜色ColorPicker
UI->>UI : 更新颜色状态#RRGGBB
UI->>UI : 调整透明度Slider
UI->>UI : 计算 Alpha 十六进制值
UI->>UI : 生成 #AARRGGBB 结果
UI->>Clipboard : 复制 HEX 值
Clipboard-->>UI : 复制成功/失败
UI->>Clipboard : 复制 ARGB 值
Clipboard-->>UI : 复制成功/失败
UI->>UI : 显示颜色预览棋盘格背景
```

**图表来源**
- [src/tools/color/ColorPicker/index.tsx:18-43](file://src/tools/color/ColorPicker/index.tsx#L18-L43)
- [src/tools/color/ColorPicker/index.tsx:23-24](file://src/tools/color/ColorPicker/index.tsx#L23-L24)
- [src/tools/color/ColorPicker/index.tsx:26-43](file://src/tools/color/ColorPicker/index.tsx#L26-L43)
...
Registry-->>Routes : 工具列表包含颜色工具
Routes-->>Routes : 为每个工具生成路由
Layout->>Registry : getToolCategories()
Registry-->>Layout : 分类列表包含 color 分类
...
- [src/tools/registry.ts:34-49](file://src/tools/registry.ts#L34-L49)
...
## 依赖分析
- 外部依赖：React、Ant Design、crypto-js、Zustand、@tauri-apps/plugin-http、mammoth、png2ico、to-ico 等。
- 内部依赖：工具注册表聚合 JSON、加密、格式转换、颜色与文本工具；路由与布局依赖注册表；工具组件依赖各自工具函数与状态存储。
...
Package --> Png2Ico["png2ico"]
Package --> ToIco["to-ico"]
...
Registry --> ColorTools["tools/color/index.tsx"]
...
DocxUtils --> Mammoth
ColorPicker["tools/color/ColorPicker/index.tsx"] --> Antd
...
- [src/tools/color/ColorPicker/index.tsx:1-2](file://src/tools/color/ColorPicker/index.tsx#L1-L2)
...
- 颜色工具：使用 Ant Design ColorPicker 组件，性能优化良好，支持实时预览。
...
- 颜色工具故障：
  - 确认浏览器支持 ColorPicker 组件和 Clipboard API。
  - 检查透明度滑块是否正常工作，数值范围 0-100。
  - 查看颜色预览是否正确显示棋盘格透明度背景。
  - 确认复制功能正常，检查剪贴板权限。
- HashCode 计算异常：
  - 确认输入字符串不为空。
...
## 结论
XKUtil 工具系统通过注册表驱动的分类与动态路由机制，实现了清晰的模块划分与良好的扩展性。JSON、加密、格式转换、颜色与文本处理五大模块分别提供了实用的数据处理、安全能力、文档转换、视觉设计与配置生成能力，配合懒加载与状态持久化，兼顾了性能与用户体验。版本 0.6.0 新增的颜色工具模块进一步完善了工具系统的视觉设计能力，特别包含 ColorPicker 工具，支持颜色选择、透明度调整、实时预览和多种输出格式，新增了 Ant Design ColorPicker 组件依赖。未来可在现有基础上完善关键字搜索、工具权限与审计日志等能力，进一步增强企业级可用性。
...
- 颜色工具函数
  - opacityToHex(opacity): 将透明度百分比转换为十六进制字符串。
  - ColorPickerTool: 颜色选择器组件，支持颜色选择、透明度调整、实时预览和复制功能。
- 配置生成器函数
...
**章节来源**
- [src/tools/registry.ts:16-49](file://src/tools/registry.ts#L16-L49)
...
- [src/tools/color/ColorPicker/index.tsx:6-12](file://src/tools/color/ColorPicker/index.tsx#L6-L12)
- [src/utils/config-generator.ts:1-117](file://src/utils/config-generator.ts#L1-L117)
...
- 在新工具中使用格式转换工具
  ...
  - 展示图片处理统计与转换警告信息。
- 在新工具中使用颜色工具
  - 引入 ColorPicker 组件和 opacityToHex 函数。
  - 在 UI 中提供颜色选择器、透明度滑块和预览区域。
  - 实现 HEX 值和 ARGB 值的复制功能。
  - 处理剪贴板权限和错误提示。
...
- [src/utils/docx.ts:11-47](file://src/utils/docx.ts#L11-L47)
- [src/tools/color/ColorPicker/index.tsx:14-138](file://src/tools/color/ColorPicker/index.tsx#L14-L138)
- [src/tools/color/ColorPicker/index.tsx:6-12](file://src/tools/color/ColorPicker/index.tsx#L6-L12)