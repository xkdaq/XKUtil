...
## 更新摘要
**变更内容**
- 新增翻译工具（Translate），支持百度、有道、阿里、腾讯四大翻译服务商
- 新增事件映射工具（EventMapping），基于MD5生成行为打点配置映射
- 扩展工具注册表，包含翻译工具和事件映射工具的完整集成
- 新增翻译状态管理与配置存储，支持多服务商配置持久化
- 版本升级至 0.4.0，显著增强了工具系统的功能完整性
...
## 简介
本文件为 XKUtil 工具系统的技术文档，围绕工具注册表机制、工具分类与动态注册流程、工具接口规范、JSON 工具、加密工具与文本处理三大模块的架构与实现进行深入解析，并提供工具开发规范、扩展指南、API 文档与集成示例，帮助开发者快速理解与扩展该工具平台。

**更新** 版本 0.4.0 新增了翻译工具和事件映射工具，进一步完善了工具系统的功能覆盖，新增了多语言翻译能力和行为打点配置生成功能。

## 项目结构
XKUtil 采用前端单页应用架构，工具系统位于 src/tools 下，按功能域划分为 JSON 工具、加密工具与文本处理三类；通用工具函数位于 src/utils；路由与侧边菜单由布局组件负责渲染；状态管理通过 Zustand 的轻量 store 实现。
...
**更新** 新增了翻译工具模块和事件映射工具，包含多服务商翻译能力和行为打点配置生成功能。
...
Registry-->>Layout : 返回分类与工具列表包含翻译和事件映射
Layout-->>Browser : 渲染侧边菜单
Browser->>Routes : 访问工具路由
Routes->>Registry : 获取全部工具
Registry-->>Browser : 返回包含 text 和 crypto 工具的完整列表
...
### 工具注册表与分类系统
- 职责：聚合 JSON、加密与文本处理工具，按 category 构建分类树，提供分类与全量工具查询。
- 关键点：
  - 使用 Map 去重与聚合，确保分类唯一性与工具列表完整性。
  - 分类名映射函数支持中文化显示，包含 text 和 crypto 类别。
  - 工具路径与组件懒加载结合，保证首屏性能。

```mermaid
flowchart TD
Start(["开始"]) --> Collect["收集 allTools<br/>合并 JSON、加密与文本工具包含翻译和事件映射"]
...
Registry-->>Routes : 工具列表包含翻译和事件映射工具
Routes-->>Routes : 为每个工具生成路由
Layout->>Registry : getToolCategories()
Registry-->>Layout : 分类列表包含 text 和 crypto 分类
...
## 依赖分析
- 外部依赖：React、Ant Design、crypto-js、Zustand、@tauri-apps/plugin-http 等。
- 内部依赖：工具注册表聚合 JSON、加密与文本工具；路由与布局依赖注册表；工具组件依赖各自工具函数与状态存储。
...
## 结论
XKUtil 工具系统通过注册表驱动的分类与动态路由机制，实现了清晰的模块划分与良好的扩展性。JSON、加密与文本处理三大模块分别提供了实用的数据处理、安全能力和配置生成能力，配合懒加载与状态持久化，兼顾了性能与用户体验。版本 0.4.0 新增的翻译工具和事件映射工具进一步完善了工具系统的功能覆盖，新增了多语言翻译服务能力、行为打点配置生成功能和多服务商翻译支持，为开发者提供了更全面的工具集。未来可在现有基础上完善关键字搜索、工具权限与审计日志等能力，进一步增强企业级可用性。
